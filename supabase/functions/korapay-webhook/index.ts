import { createClient } from "npm:@supabase/supabase-js@2";
  ...
  // Korapay signs webhooks using HMAC-SHA256 with the ENCRYPTION key (not the secret key)
  ...
  Deno.serve(async (req) => {
    try {
      ...
      // Reject requests with no signature header entirely (all genuine Korapay webhooks include it)
      if (!sig) {
        console.warn("korapay-webhook: missing x-korapay-signature header — rejecting");
        return new Response("Unauthorized", { status: 401 });
      }

      // Reject if signature doesn't match (requires encryption key to verify)
      if (!encryption_key) {
      ...
      if (!order.is_installment) {
        await supabase
          ...
          .eq("id", orderId);
        console.log("korapay-webhook: order", orderId, "marked paid");
      } else {
        ...
        if (inst) {
          ...
          console.log("korapay-webhook: installment part", partNumber, "recorded for order", orderId, "| remaining:", remaining);
        }
      }
